...
// ---------------------------------------------------------------------------
// Contexto — permite abrir o modal de compra a partir dos cards de ingresso
// ---------------------------------------------------------------------------
type Mode = "lote" | "individual";
type Ctx = { open: (mode?: Mode, initialQty?: number) => void };
...
// Formata somente CPF (000.000.000-00)
const maskCpf = (value: string) => maskDoc(value.replace(/\D/g, "").slice(0, 11));

const inputCls =
  "mt-2 w-full rounded-md border border-ink-200 bg-ink-0 px-4 py-3 text-brand-900 placeholder:text-ink-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-subtle/40";

// ---------------------------------------------------------------------------
// Botão de compra — abre o modal no modo escolhido
// ---------------------------------------------------------------------------
...
export function TicketModalProvider({ children }: { children: React.ReactNode }) {
  ...
  const open = useCallback((m: Mode = "lote", initialQty?: number) => {
    setMode(m);
    setQty(initialQty && initialQty > 0 ? clampQty(initialQty) : m === "individual" ? 1 : TICKET.defaultQuantity);
    setIsOpen(true);
  }, []);
  ...
  const noun = isIndividual ? "ingresso" : "convite";
              ...
              <div className="flex items-start gap-4 rounded-t-lg border-b border-ink-100 bg-ink-0 p-6">
                ...
                <div className="flex-1">
                  <h2 id="ticket-modal-title" className="font-heading text-2xl text-brand-900">
                    {isIndividual ? "Comprar ingresso" : "Adquirir ingressos"}
                  </h2>
                  <p className="mt-1 text-sm text-ink-600">
                    {isIndividual
                      ? "Preencha seus dados e finalize a compra do seu ingresso."
              ...
              <div className="space-y-4 p-6">
                ...
                <div className="rounded-lg border border-ink-100 bg-ink-0 p-4">
                  ...
                  {isIndividual ? (
                    <>
                      <p className="mt-1 text-sm text-ink-600">
                        Usaremos esses dados para emitir e enviar o seu ingresso.
                      </p>
                      <label className="mt-4 block">
                        <span className="text-sm font-semibold text-brand-900">Nome completo</span>
                        <input
                          value={name}
                      ...
                      <label className="mt-4 block">
                        <span className="text-sm font-semibold text-brand-900">E-mail</span>
                        <input
                          type="email"
                          value={email}
                      ...
                      <label className="mt-4 block">
                        <span className="text-sm font-semibold text-brand-900">CPF</span>
                        <input
                          value={doc}
                ...
                {/* Quantidade */}
                <div className="rounded-lg border border-ink-100 bg-ink-0 p-4">
                  <h3 className="font-heading text-base text-brand-900">
                    {isIndividual ? "Quantidade de ingressos" : "Quantidade de convites"}
                  </h3>
                  <p className="mt-1 text-sm text-ink-600">
                    {isIndividual
                      ? `Escolha quantos ingressos deseja — ${brl(TICKET.pricePerUnit)} por ingresso.`
                  ...
                  <div className="mt-4 flex items-start gap-2 rounded-md bg-ink-100 p-3 text-sm text-ink-600">
                    ...
                    <p>
                      ...
                      {isIndividual
                        ? `Ao concluir, você recebe ${qty} ingresso${plural} por e-mail.`
